    ...
    setup(props) {
        ...
        const submitPriceForm = () => {
            priceForm.transform((data) => {
                ...
                return data
            }).post(route('account.apartments.price.update', {
        ...
        const submitRangeForm = () => {
            rangeForm.transform((data) => {
                data.price = parseInt(data.price)
                return data
            }).post(route('account.apartments.calendar.update', {
                apartment: props.apartment.data.id,
            }, {preserveScroll: true}), {
                onSuccess: () => {
                ...
                <div class="px-6 pt-1 w-full lg:w-5/12 form sticky top-24">
                    <template v-if="!(rangeForm.start && rangeForm.end)">
                        ...
                        <div class="mt-8 flex flex-col gap-3">
                            <Input v-model="priceForm.weekdays_price" type="number"
                                   :error="priceForm.errors.weekdays_price"
                                   label="Цена в будни, ₽"/>
                            <Input v-model="priceForm.weekends_price" type="number"
                                   :error="priceForm.errors.weekends_price"
                                   label="Цена в выходные, ₽"/>
                            <ButtonComponent :disabled="!priceForm.isDirty" class="mt-6" label="Сохранить"
                    ...
                    <template v-else>
                        ...
                        <div class="mt-8 flex flex-col gap-3">
                            <Input v-model="rangeForm.price" type="number"
                                   :error="rangeForm.errors.price"
                                   label="Цена, ₽"/>
                            <ButtonComponent :disabled="!rangeForm.isDirty" class="mt-6" label="Сохранить"
