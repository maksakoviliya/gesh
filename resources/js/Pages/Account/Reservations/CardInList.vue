...
const handleClickApartment = () => {
    router.visit(route('apartment', {
        apartment: props.reservation.apartment?.id,
    }))
}

const handleClickReservation = () => {
    router.visit(route('account.reservations.view', {
...
<template>
    <div class="rounded-lg border cursor-pointer hover:shadow-lg hover:bg-gray-50 transition" @click="handleClickReservation">
        <div class="p-4 flex items-stretch gap-2" @click="handleClickApartment">
            <div
        ...
        <dl class="divide-y border-t">
            ...
            <div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
                ...
                <dd class="mt-1 font-medium leading-6 text-neutral-600 ">
                    <template v-if="props.reservation.children">
                        {{ props.reservation.children }} детей
                    </template>
                    <template v-else>
                        -
                    </template>
            ...
            <div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
                ...
                <dd class="mt-1 font-medium leading-6 text-neutral-600 flex items-center gap-2">
                    ...
                    <Badge :label="badgeText" :type="badgeType" />
                </dd>
            </div>
        </dl>
    </div>
