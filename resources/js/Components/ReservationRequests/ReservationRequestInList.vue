<script setup>
	import { Link, router } from '@inertiajs/vue3'
	import { computed } from 'vue'
	import Badge from '@/Components/Badge.vue'

	const props = defineProps({
		reservation_request: Object,
	})

	const subtitle = computed(() => {
		const result = [
			`${props.reservation_request.apartment.guests}&nbsp;гостей`,
			`${props.reservation_request.apartment.bedrooms}&nbsp;спальня`,
			`${props.reservation_request.apartment.beds}&nbsp;кровать`,
			`${props.reservation_request.apartment.bathrooms}&nbsp;ванная`,
		]
		return result.filter((item) => !!item).join(' · ')
	})

	const handleClickApartment = () => {
		router.visit(
			route('apartment', {
				apartment: props.reservation_request.apartment.id,
			})
		)
	}

	const handleClickReservation = () => {
		router.visit(
			route('account.apartments.chat', {
				apartment: props.reservation_request.apartment.id,
			})
		)
	}
</script>

<template>
	<div class="rounded-lg border cursor-pointer hover:shadow-lg hover:bg-gray-50 transition">
		<div
			class="p-4 flex items-stretch gap-2"
			@click="handleClickApartment"
		>
			<div class="h-24 w-32 rounded-lg overflow-hidden">
				<img
					:src="props.reservation_request.apartment?.media[0]?.src"
					class="h-full w-full object-cover"
					alt=""
				/>
			</div>
			<div class="flex flex-col justify-start">
				<div class="text-gray-600 font-light dark:text-slate-200">
					{{
						props.reservation_request.apartment?.title ??
						props.reservation_request.apartment?.category?.title_single
					}}
					({{ props.reservation_request.apartment?.city }})
				</div>
				<div class="text-gray-400 font-light text-sm dark:text-slate-200">
					{{ props.reservation_request.apartment?.category?.title_single }}
				</div>
				<div
					class="font-light text-xs text-gray-600 mt-auto dark:text-slate-200"
					v-html="subtitle"
				></div>
			</div>
		</div>
		<dl
			class="divide-y border-t"
			@click="handleClickReservation"
		>
			<div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
				<dt class="font-light leading-6 text-gray-600">
					<div>Даты:</div>
				</dt>
				<dd class="mt-1 font-medium leading-6 text-neutral-600">
					с {{ props.reservation_request.start }} по {{ props.reservation_request.end }}
				</dd>
			</div>
			<div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
				<dt class="font-light leading-6 text-gray-600">
					<div>Ночи:</div>
				</dt>
				<dd class="mt-1 font-medium leading-6 text-neutral-600">{{ props.reservation_request.range }} ночи</dd>
			</div>
			<div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
				<dt class="font-light leading-6 text-gray-600 dark:text-slate-300">
					<div>Гости:</div>
				</dt>
				<dd class="mt-1 font-medium leading-6 text-neutral-600 dark:text-slate-400">
					{{ props.reservation_request.guests }} гостей
				</dd>
			</div>
			<div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
				<dt class="font-light leading-6 text-gray-600">
					<div>Дети:</div>
				</dt>
				<dd class="mt-1 font-medium leading-6 text-neutral-600">
					<template v-if="props.reservation_request.children">
						{{ props.reservation_request.children }} детей
					</template>
					<template v-else> - </template>
				</dd>
			</div>
			<div class="py-2 px-4 flex w-full items-baseline justify-between text-sm">
				<dt class="font-light leading-6 text-gray-600">
					<div>Стоимость:</div>
				</dt>
				<dd class="mt-1 font-medium leading-6 text-neutral-600 flex items-center gap-2">
					{{ props.reservation_request.price.toLocaleString() }}₽
					<Badge
						label="Ожидает одобрения"
						type="info"
					/>
				</dd>
			</div>
		</dl>
	</div>
</template>
